...
pub fn initialize_beacon_state_from_eth1<T: EthSpec>(
    ...
    if spec.altair_fork_slot == Some(spec.genesis_slot) {
        ...
        let next_sync_committee = Arc::new(state.get_sync_committee(state.next_epoch()?, spec)?);
        state.as_altair_mut()?.current_sync_committee = next_sync_committee.clone();
        state.as_altair_mut()?.next_sync_committee = next_sync_committee;

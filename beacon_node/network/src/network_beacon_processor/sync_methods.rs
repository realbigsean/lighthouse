...
use crate::sync::{
    manager::{BlockProcessType, ResponseType, SyncMessage},
    BatchProcessResult, ChainId,
};
...
impl<T: BeaconChainTypes> NetworkBeaconProcessor<T> {
    ...
    /// Returns an async closure which processes blob sidecars recieved via RPC.
    ///
    /// This separate function was required to prevent a cycle during compiler
    /// type checking.
    pub fn generate_rpc_blobs_process_fn(
        ...
        let process_fn = async move {
            self.process_rpc_blobs(block_root, blobs, seen_timestamp, process_type)
                .await;
        };
        Box::pin(process_fn)
    }
